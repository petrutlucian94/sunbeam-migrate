...
class LoadBalancerHandler(base.BaseMigrationHandler):
    ...
    def perform_individual_migration(
        ...
        self._wait_for_load_balancer_active(dest_lb_id, self._destination_session)
        ...
        for source_listener in source_listeners:
            ...
            if source_listener.default_pool_id:
                ...
                if source_pool and source_pool.id not in pool_id_map:
                    ...
                    self._wait_for_load_balancer_active(
                        dest_lb_id, self._destination_session
                    )
    ...
    def _wait_for_load_balancer_active(
        self, load_balancer_id: str, session, timeout: int = 300, interval: int = 5
    ):
        ...
        while elapsed < timeout:
            lb = session.load_balancer.get_load_balancer(load_balancer_id)
            if not lb:
                raise exception.NotFound(f"Load balancer not found: {load_balancer_id}")

            provisioning_status = getattr(lb, "provisioning_status", "").upper()
            if provisioning_status == "ACTIVE":
                return
            elif provisioning_status == "ERROR":
                raise exception.SunbeamMigrateException(
                    f"Load balancer {load_balancer_id} entered ERROR state"
                )

            LOG.debug(
                "Waiting for load balancer %s to be ACTIVE (current: %s)",
                load_balancer_id,
                provisioning_status,
            )
            time.sleep(interval)
        ...
        raise exception.SunbeamMigrateException(
            f"Timeout waiting for load balancer {load_balancer_id} to be ACTIVE"
        )
